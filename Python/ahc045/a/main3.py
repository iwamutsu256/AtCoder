...
def solve(input_data: Input) -> Output:
    ...
    # これを繰り返してグループ分けをする
    # グループ内の都市について占いの点の数を最大にしてそれをつなげる
    # sample.pyの占いの数をできるだけ多くしたバージョン
    group_list = [] #groupの配列
    ...
    for i in range(input_data.group_count):
        ...
        # グループ内の都市について占いから辺を作成する
        start_idx = 0
        # 一つしか都市が残っていない場合、辺が作れないので、都市が２つ以上残っていることがループの条件
        while start_idx != len(group.cities)-1:
            # 現在残っている都市の数はlen(group.cities)-1-start_idx
            # 占い都市の数マックスよりも都市の数が少ないとき
            if input_data.query_max_cities_size > len(group.cities)-1-start_idx:
                # 都市の数だけの占いをする
                edges = forecast(group.cities[start_idx:])
                start_idx = len(group.cities)-1
                # 残っている都市の数が占い都市の数マックス＋１の場合
                # は存在しないかも
                """
                elif len(group.cities)-1-start_idx == input_data.query_max_cities_size + 1:
                    # 都市二つ残して占う
                    edges = forecast(group.cities[start_idx:len(group.cities)-2])
                    start_idx = len(group.cities)-2
                """
            else:
                #マックス分占う
                edges = forecast(group.cities[start_idx:start_idx+input_data.query_max_cities_size])
                start_idx += input_data.query_max_cities_size-1
            # すべての辺をグループに追加する
            for edge in edges:
