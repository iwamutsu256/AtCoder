...
def count_score(query_ans):
    """
    スコアを計算する関数
    
    Args:
        query_ans (list[Edge]): 占い結果の辺の配列
    
    Returns:
        score (int): スコア
    """
    score = 0
    for edge in query_ans:
        # 都市のインデックスでソートしておく
        query_cities = sorted(edge[0],key=lambda city: city.idx)
        # すべての辺の組み合わせについて長さを調べ、ソートする
        real_edges = []
        # すべての辺(u,v)に対して、u<vが成り立っている
        real_edges.append(Edge(query_cities[0],query_cities[1]))
        real_edges.append(Edge(query_cities[1],query_cities[2]))
        real_edges.append(Edge(query_cities[0],query_cities[2]))
        real_edges.sort(key=lambda edge: edge.predicted_length())
        # クエリの応答の辺はインデックスがedgeの1~3であることに注意してスコアをカウントする
        count = 0
        if real_edges[0] == edge[2] and real_edges[1] == edge[1]:
            count == 1
        else:
            for i in range(2):
                if real_edges[i] != edge[i+1]:
                    count += 1
        score += count
    return score
...
def update_points(input_data: Input) -> Input:
    ...
    # 四角形の面積でソート
    sorted_cities = sorted(cities, key=lambda city: city.predicted_S)
    small_error_cities = sorted_cities[:input_data.cities_count//4]
    # 40個に対して10回ずつ占いをする
    for i in range(40):
        query_ans = []
        biggest_error_city = sorted_cities.pop()
        for j in range(10):
            a = random.choice(small_error_cities)
            b = random.choice(small_error_cities)
            while a == b:
                b = random.choice(small_error_cities)
            query_cities = [biggest_error_city,a,b]
            query_ans.append(forecast(query_cities))
        # とりあえず、0.025秒山登りをしてみる(合計1秒)
        start_time = time.time()
        time_limit = 0.025
        current_score = count_score(query_ans)
        iteration = 0
        while True:
            if time.time() - start_time >= time_limit or current_score == 0:
                break
            old_predicted_pos = query_cities[0].predicted_pos
            new_predicted_pos = Point(random.uniform(query_cities[0].left_up.x,query_cities[0].right_down.x),random.uniform(query_cities[0].left_up.y,query_cities[0].right_down.y))
            # 予測位置を変更してみる
            cities[query_cities[0].idx].predicted_pos = new_predicted_pos
            # スコアを計算する
            new_score = count_score(query_ans)
            if current_score > new_score:
                # スコアが上がったら、予測位置を変更する
                current_score = new_score
            else:
                # スコアが下がったら、元に戻す
                cities[query_cities[0].idx].predicted_pos = old_predicted_pos
            # 試行回数をカウントする
            iteration += 1
            if iteration % 10 == 0:
                print(f"iteration: {iteration}, score_count: {current_score}",file=sys.stderr)
        print(f"iteration: {iteration}, score_count: {current_score}",file=sys.stderr)
