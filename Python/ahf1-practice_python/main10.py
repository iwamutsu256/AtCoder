...
def solve_greedy(input_data: Input, restaurant_g: 'Point', destination_g: 'Point') -> Output:
    ...
    for i in range(input_data.pickup_count):
        ...
        for j in candidates:
            ...
            distance = input_data.restaurants[j].dist(r_g)*input_data.destinations[j].dist(d_g)*input_data.restaurants[j].dist(input_data.office)*input_data.destinations[j].dist(input_data.office)
            if distance <= min_dist:
                min_dist = distance
                nearest_cand = j
                #min_r_g = r_g.times(len(orders)+1).plus(input_data.restaurants[j]).times(1/(len(orders)+2))
                #min_d_g = d_g.times(len(orders)+1).plus(input_data.destinations[j]).times(1/(len(orders)+2))
        #最小だったものを選び、重心を変更する
...
def main():
    ...
    # 問題を解く
    #レストランの重心
    rest_g = Point(500,500)
    #配達先の重心
    dest_g = Point(300,300)
