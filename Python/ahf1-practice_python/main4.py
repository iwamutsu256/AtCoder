...
class Output:
    ...
    def __init__(self, orders: list[int], route: list[Point], destination_dict: dict[int], restaurant_dict: dict[int], time: float, candidate: list[int]):
        """
        出力データを構築する
...
def solve_greedy(input_data: Input) -> Output:
    ...
    total_dist = 0                       # 総移動距離
    candidates = list(set([int(x) for x in range(input_data.order_count)])-set(orders))
    ...
    return Output(orders, route, destination_dict, restaurant_dict, finish_time-start_time,candidates)
...
def solve_simulated_annealing(input_data: Input, output_data_greedy: Output) -> Output:
    ...
    candidates = list(output_data_greedy.candidate)
    ...
    while True:
        ...
        if iteration % 5 == 1:#ある注文番号のレストランと配達先をほかの注文番号に変える
            ...
            if delta <=0 or random.random() < math.exp(-delta / current_temperature):
                ...
                current_dist += delta
        elif iteration % 2 == 1:#レストランを選んで配達先よりも前に挿入
            i = restaurant_dict[rand_orders]
    ...
    return Output(orders, route, destination_dict, restaurant_dict, time.time()-start_time, candidates)
