...
#A6から更新する
#動作のスコアをTによらない目標利益を達成するまでのターンで比較するようにした。
#2回目以降の探索を近傍の探索をやめた
...
#このクラスでは盤面の管理のみを行う
class Field:
    def __init__(self,N: int,M: int,distance:list[int],home_point:list[list[set]],workspace_point:list[list[set]],home:list[Pos],workspace:list[Pos]):
        self.N = N
        self.M = M
        ...
        self.railway_done = [False for _ in range(M)]
        self.uf = UnionFind(N)
        self.distance = distance
        self.home = home
        self.workspace = workspace
        self.gpp = [[EMPTY for _ in range(N)] for _ in range(N)]
    ...
    def nearest_station(self,Pos):
        ...
        for item in self.station_pos:
            heapq.heappush(calc_dis,[manhattan_distance(Pos,item),item])
        flag = True
        while flag:
            dis = heapq.heappop(calc_dis)
            _,_,real_dis = self.a_star(Pos,dis[1])
            if real_dis - dis[0] > 5:
                heapq.heappush(calc_dis,[real_dis,dis[1]])
            else:
                flag = False
        return dis[1]

    #self.cand_homeを更新する関数
    def get_cand_home(self,station_pos:list):
        cand_home = set()
        for item in station_pos:
            for i in self.neighbors(item):
                home_point_cand = self.home_point[i[0]][i[1]]
                for item2 in home_point_cand:
                    if not self.is_connected(self.home[item2],self.workspace[item2]):
                        cand_home.add(item2)
                #cand_home.update(self.home_point[i[0]][i[1]])
        self.cand_home = cand_home
        return

    #self.cand_workspaceを更新する関数
    def get_cand_workspace(self,station_pos:list):
        cand_workspace = set()
        for item in station_pos:
            for i in self.neighbors(item):
                workspace_point_cand = self.workspace_point[i[0]][i[1]]
                for item2 in workspace_point_cand:
                    if not self.is_connected(self.home[item2],self.workspace[item2]):
                        cand_workspace.add(item2)
                #cand_workspace.update(self.workspace_point[i[0]][i[1]])
        self.cand_workspace = cand_workspace
        return
    #cand_homeとcand_workspaceを計算する関数
    def calc_cand(self,station_pos:list):
        self.get_cand_home(station_pos)
        self.get_cand_workspace(station_pos)
        cand = self.cand_home & self.cand_workspace
        self.cand_home -= cand
        self.cand_workspace -= cand
        return
    
    #self.cand_homeとself.cand_workspaceを更新せずに近くの組み合わせを返す関数
    #片側はすでに路線に接続されている
    def near_cand(self,p:Pos):
        #地点Ⅰ
        cand1 = set()
        candi = []
        for i in self.neighbors(p):
            cand1.update(self.workspace_point[i[0]][i[1]])
        for item in cand1:
            if self.uf._find_root(self.home[item][0]*self.uf.n+self.home[item][1]) == self.home[item][0]*self.uf.n+self.home[item][1]:
                candi.append(item)
        cand1 = set()
        for i in self.neighbors(p):
            cand1.update(self.home_point[i[0]][i[1]])
        for item in cand1:
            if self.uf._find_root(self.workspace[item][0]*self.uf.n+self.workspace[item][1]) == self.workspace[item][0]*self.uf.n+self.workspace[item][1]:
                candi.append(item)
        if len(candi) > 0:
            return candi[0]
        else:
...
class Solver:
    def __init__(self, N: int, M: int, K: int, T: int, home:list[Pos], workspace:list[Pos],distance:list[int],home_point:list[list[set]],workspace_point:list[list[set]]):
        ...
        self.home = home
        self.workspace = workspace
        self.field = Field(N,M,distance,home_point,workspace_point,home,workspace)
        self.distance = distance
        self.home_point = home_point
        self.workspace_point = workspace_point
        self.turn = 0
    ...
    def first_station(self):
        ...
        msg = None  # 初期化
        for i in range(self.M):
            if self.money - COST_STATION*2 - COST_RAIL*(self.distance[i]-1) >= 0:
                ...
                #                msg = [j, k, i]
                conbi_point = self.distance[i] * (self.field.grid_pos_point(self.field.home[i][0],self.field.home[i][1])+self.field.grid_pos_point(self.field.workspace[i][0],self.field.workspace[i][1]))
                if  conbi_point > maximum:
                    maximum = conbi_point
                    msg = [self.field.home[i],self.field.workspace[i],i]
            else:
                pass
        if msg is None:
        ...
        self.field.railway_done[msg[2]] = True
        return msg
    

    #candidate再計算用
    def recalc_cand(self):
        if len(self.candidate) == 0:
            return []
        candidate = heapq.heappop(self.candidate)
        #is_railway_doneかを調べる
        if self.field.is_connected(self.home[candidate[5]],self.workspace[candidate[5]]) and self.field.railway_done[candidate[5]] == False:
            self.field.railway_done[candidate[5]] = True
        if self.field.railway_done[candidate[5]]:
            return self.recalc_cand()
        _,pre_path,length_path = self.field.a_star(candidate[2],candidate[1])
        if candidate[3] == length_path and candidate[4] == pre_path:
            return candidate
        else:
            item = candidate[5]
            if item != -1:
                new_score = (self.field.distance[item]*self.field.grid_pos_point(candidate[2][0],candidate[2][1]))/((length_path-1)+(TARGET+COST_STATION+COST_RAIL*(length_path-2))/self.field.distance[item])
                heapq.heappush(self.candidate,[-1*new_score,candidate[1],candidate[2],length_path,pre_path,item])
            return self.recalc_cand()

    def before_select(self):
        if len(self.field.station_pos) == 2:
            new_station = self.field.station_pos
        else:
            new_station = [self.field.station_pos[-1]]
        self.field.calc_cand(new_station)
        ...
        for item in cand_home:
            ...
            length = 0
            path = []
            nearest_sta = self.field.nearest_station(self.workspace[item])
            _,pre_path,length_path = self.field.a_star(self.workspace[item],nearest_sta)
            if length_path != 0:
                maximum = self.field.grid_pos_point(self.workspace[item][0],self.workspace[item][1])
                sta_pos = self.workspace[item]
                length = length_path
                path = pre_path
                new_score = (self.field.distance[item]*maximum)/((length-1)+(TARGET+COST_STATION+COST_RAIL*(length-2))/self.field.distance[item])
                heapq.heappush(self.candidate,[-1*new_score,nearest_sta,sta_pos,length,path,item])
        for item in cand_workspace:
            ...
            length = 0
            path = []
            nearest_sta = self.field.nearest_station(self.home[item])
            _,pre_path,length_path = self.field.a_star(self.home[item],nearest_sta)
            if length_path != 0:
                maximum = self.field.grid_pos_point(self.home[item][0],self.home[item][1])
                sta_pos = self.home[item]
                length = length_path
                path = pre_path
                new_score = (self.field.distance[item]*maximum)/((length-1)+(TARGET+COST_STATION+COST_RAIL*(length-2))/self.field.distance[item])
                heapq.heappush(self.candidate,[-1*new_score,nearest_sta,sta_pos,length,path,item])
    ...
    def select_station(self):
        ...
        cand_home = self.field.cand_home
        cand_workspace = self.field.cand_workspace
        if self.actions[self.turn-1].type == 0:
            for item in cand_home:
                #この駅は固定
                #固定ではないことが発覚
                #その地点から一番近い駅に結んだほうが効率がいい
        ...
        #できないならできるようになるまで待機
        #そもそも利益が出ないなら一生待機
        candidate = self.recalc_cand()
        #score = self.money + self.income*(self.T-self.turn)
        if candidate == [] or candidate == None:
            while len(self.actions) < self.T:
                self.build_nothing()
        else:
            if ((candidate[3]-1)+(COST_STATION+COST_RAIL*(candidate[3]-2))/self.field.distance[candidate[5]])+self.turn >= self.T:
                #一生待機
                while len(self.actions) < self.T:
                    self.build_nothing()
            else:
                if self.money + self.income*(candidate[3]-2) >= COST_STATION + (candidate[3]+2)*COST_RAIL:
                    ...
                    while self.money + self.income*(candidate[3]-2) < COST_STATION + (candidate[3]+2)*COST_RAIL and len(self.actions) < self.T:
                        self.build_nothing()
                    heapq.heappush(self.candidate,candidate)
