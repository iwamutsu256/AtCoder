...
#このクラスでは盤面の管理のみを行う
class Field:
    def __init__(self,N: int,distance:list[int],home_point:list[list[set]],workspace_point:list[list[set]],home:list[Pos],workspace:list[Pos]):
        self.N = N
...
class Solver:
    def __init__(self, N: int, M: int, K: int, T: int, home:list[Pos], workspace:list[Pos],distance:list[int],home_point:list[list[set]],workspace_point:list[list[set]]):
        ...
        self.home = home
        self.workspace = workspace
        self.field = Field(N,distance,home_point,workspace_point,home,workspace)
        self.distance = distance
        self.home_point = home_point
        self.workspace_point = workspace_point
        self.turn = 0
    ...
    def recalc_cand(self):
        ...
        if candidate[3] == length_path and candidate[4] == pre_path:
            ...
            if item != -1:
                new_score = self.money-(COST_STATION+COST_RAIL*(length_path-2))+self.income*(self.T-self.turn)+(801-self.turn-length_path)*self.field.distance[item]
                heapq.heappush(self.candidate,[-1*new_score,candidate[1],candidate[2],length_path,pre_path])
            self.recalc_cand()
    ...
    def select_station(self):
        ...
        for item in cand_home:
            ...
            heapq.heappush(self.candidate,[-1*new_score,nearest_sta,sta_pos,length,path])
            #if len(candidate) == 0:
        ...
        #追加で上のfor i in stationの部分にもheapqを導入したい
