...
def solve_cleaning_duty_simulated_annealing_v3(N, L, T):
    """
    焼きなまし法を用いた掃除当番最適化 (誤差が大きい社員に着目した近傍探索)
    N: 社員数
    L: 週数
    T: 各社員の掃除当番回数の目標値
    """
    ...
    for iteration in range(max_iterations):
        ...
        worst_indices = np.argsort(errors)[::-1] # 誤差が大きい順にソート

        # 誤差が大きい社員に関連する遷移を変更
        target_employee = worst_indices[0] # 最も誤差が大きい社員
        changing_employee = random.randint(0, N - 1) # 変更する社員

        # target_employeeへの遷移を変更する確率を高める
        if random.random() < 0.7:
            if random.random() < 0.5:
                new_a[changing_employee] = random.choice(np.where(errors > np.mean(errors))[0]) # 誤差が大きい社員へ変更
            else:
                new_b[changing_employee] = random.choice(np.where(errors > np.mean(errors))[0]) # 誤差が大きい社員へ変更
        else:
            # ランダムな変更も行う
            if random.random() < 0.5:
...
def main():
    ...
    a, b, _ = solve_cleaning_duty_simulated_annealing_v3(N, L, T)
