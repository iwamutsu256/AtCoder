    ...
    for i in range(input_data.pickup_count):
        ...
        for j in candidates:
            ...
            for k in true_candidate:
                distance += input_data.restaurants[j].dist(input_data.restaurants[k])**2
                distance += input_data.destinations[j].dist(input_data.destinations[k])**2
                distance += input_data.restaurants[j].dist(input_data.destinations[k])**2
                distance += input_data.destinations[j].dist(input_data.restaurants[k])**2
    ...
    while True:
        ...
        if iteration & 1:#レストランを選んで配達先よりも前に挿入
            ...
            if current_dist >= new_dist:
                ...
                if random.random() < math.exp((current_dist - new_dist) / current_temperature):
                    current_dist = new_dist
                else:
                    point_to_move = route.pop(j)
                    route.insert(i, point_to_move)
                    update_index(destination_dict, restaurant_dict, j, i)
                    restaurant_dict[rand_orders] = i
            ...
            if current_dist >= new_dist:
                ...
                if random.random() < math.exp((current_dist - new_dist) / current_temperature):
                    ...
                    route.insert(i, point_to_move)
                    update_index(destination_dict, restaurant_dict, j, i)
                    destination_dict[rand_orders] = i
        ...
        #if current_dist >= new_dist: ## put your code here ##
        #    current_dist = new_dist
        #else:
        #    if random.random() < math.exp((current_dist - new_dist) / current_temperature):
        #        current_dist = new_dist
