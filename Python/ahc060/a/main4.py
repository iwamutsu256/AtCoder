...
class IceCreamSolver:
            ...
            if self.move_queue:
                ...
                if best_path:
                    ...
                    if self.plan_flavor_change():
                        ...
                        # C. 味変もできない -> 別のショップへ移動して状況を変える（リカバリー）
                        # その場しのぎの移動ではなく、ターゲットを決めて移動計画を立てる
                        if self.plan_recovery_move():
                            action = self.move_queue.popleft()
                        else:
                            # 万が一移動計画も立たない場合（孤立など）、ランダムに動く
                            # ただしUターンは避ける
    ...
    def plan_recovery_move(self) -> bool:
        """
        手詰まり時、現在地以外のショップへ移動して状況をリセットするための計画を立てる。
        移動パスを move_queue に追加する。
        """
        # ターゲット決定: 現在地以外のショップからランダムに、あるいは近場のショップへ
        # ここでは「現在地以外のショップ」を目指すことで場所を変える
        if self.is_shop(self.current_pos):
            candidates = [s for s in range(self.K) if s != self.current_pos]
        else:
            candidates = list(range(self.K))
            
        if not candidates: return False # K=1かつ現在地がショップなら移動しようがない

        # シャッフルしてトライ（ランダム性を導入してループ回避）
        ...
        for target_shop in candidates:
            ...
            while queue:
                curr, path = queue.popleft()
                if len(path) > 30: continue # 遠すぎるなら諦める
                
                # ゴール
                if curr == target_shop:
                    # パスが見つかった
                    # pathには [next_node, ..., target_shop] (移動順) を入れる
                    for node in path:
                        self.move_queue.append(node)
                    return True
                ...
                for neighbor in self.adj[curr]:
                    ...
                    # 途中で別のショップを通らないようにする（強制納品回避のため）
                    # ただしリカバリーなので、ターゲット以外のショップを通ってもいいが、
                    # シンプルにターゲットに直行するルートを探す方が安全
                    if self.is_shop(neighbor) and neighbor != target_shop:
                        continue
