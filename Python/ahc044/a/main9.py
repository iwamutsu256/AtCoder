...
def solve_cleaning_duty_simulated_annealing_v4(N, L, T):
    """
    焼きなまし法を用いた掃除当番最適化 (計算量削減)
    N: 社員数
    L: 週数
    T: 各社員の掃除当番回数の目標値
    """
    ...
    for iteration in range(max_iterations):
        ...
        if random.random() < 0.7:
            if random.random() < 0.5:
                new_a[changing_employee] = random.choice(np.where(errors > np.mean(errors))[0])
            else:
                new_b[changing_employee] = random.choice(np.where(errors > np.mean(errors))[0])
        else:
            # ランダムな変更も行う
            if random.random() < 0.5:
...
def main():
    ...
    a, b, _ = solve_cleaning_duty_simulated_annealing_v4(N, L, T)
