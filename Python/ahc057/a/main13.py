...
# 1フェーズあたりのターン数 (1000 / 29)
PHASE_DURATION = 1000 / 29
...
def solve():
    ...
    time_budget_per_phase = (TOTAL_TIME_LIMIT - 0.1) / NUM_GROWTH_PHASES

    # # --- フェーズ時間割の計算 (線形増加) ---
    # # 前半は短く、後半は長く確保する
    # # 重み: 20, 21, ..., 48 (計29フェーズ) -> 比率 20:48 ≒ 1:2.4
    # weights = [20 + i for i in range(NUM_GROWTH_PHASES)]
    ...
    for phase in range(NUM_GROWTH_PHASES):
        ...
        while True:
            sa_iter += 1
            if (sa_iter & 31) == 0:
                if time.time() - phase_start_time > time_budget_per_phase:
                    break
