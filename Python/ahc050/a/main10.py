# 焼きなまし法によるPの最適化（改良スコア関数・複数近傍対応・デバッグ付き）
...
def evaluate_partial(P, S, prob, begin):
    ...
    for turn in range(begin, len(P)):
        x, y = P[turn]
        flat = [p for row in prob for p in row if p > 0]
        total_prob = sum(flat)
        avg = total_prob / len(flat) if flat else 0
        var = sum((p - avg)**2 for p in flat)
        score += total_prob + 0.05 * math.sqrt(var)  # 分散による拡がりを好む
        #if turn % 100 == 0:
        #    print(f"[DEBUG] turn={turn}, total_prob={total_prob:.6f}, var={var:.6f}", file=sys.stderr)
        prob[x][y] = 0.0
        S[x][y] = "#"
        prob = diffuse(prob, S)
    return score, prob
...
def simulated_annealing(P_init, S_orig, time_limit=1.4):
    ...
    for turn in range(n):
        ...
        avg = total_prob / len(flat) if flat else 0
        var = sum((p - avg)**2 for p in flat)
        score += total_prob + 0.05 * math.sqrt(var)
