...
def solve_reduce(input_data: Input, select_orders: list[int]) -> Output:
    ...
    output_data_greedy = solve_greedy(input_data, selected_order)
    finish_time = time.time()
    return Output(output_data_greedy.orders,output_data_greedy.route,output_data_greedy.destination_dict,output_data_greedy.restaurant_dict,finish_time-start_time)
    ...
    while True:
        ...
        if iteration & 1:#レストランを選んで配達先よりも前に挿入
            ...
            if delta <= 0 or random.random() < math.exp(-delta / current_temperature):
                ...
                current_dist += delta
                #Output(orders, route, destination_dict, restaurant_dict, time.time()-start_time).print_output()
